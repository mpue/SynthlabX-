...
public:
    ...
    void setupChannels() {
        ...
        if (deviceManager->getCurrentAudioDevice() != NULL  ) {
            ...
            numInputChannels = 2;//deviceManager->getCurrentAudioDevice()->getInputChannelNames().size();
            numOutputChannels = 2;//  deviceManager->getCurrentAudioDevice()->getOutputChannelNames().size();
            
            numActiveInputs = 2; //  getNumActiveChannels(activeInputChannels.toInteger());
            numActiveOutputs = 2; //  getNumActiveChannels(activeOutputChannels.toInteger());
        }
